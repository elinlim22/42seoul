...
/*   Updated: 2022/12/07 16:04:27 by hyeslim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	piper(t_pipe *pipe, t_args *args)
{
	...
	i = 2 + pipe->hd;
	while (i < args->argc - 2)
		forker(args->argv, args->argv[i++], args->envp);
	dup2(pipe->fd_out, 1);
	//execve함수 자리
}

void	forker(char **argv, char *cmd, char **envp)
{
	int	fd[2];
	pid_t	pid;
	...
	if (pid == CHILD) //자식 프로세스
	{
		...
		close(fd[0]);
		//execve함수 자리
		execve_path(pipe, cmd, argv, envp);
	}
	else
	{
...
int	main(int argc, char *argv[], char **envp)
{
	...
	else
	{
		if (!ft_strncmp(argv[1], "here_doc", 8))
		{
			...
			pipe.fd_out = opener(argv[argc - 1], STDOUT_HD);
